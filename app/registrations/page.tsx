...
export default function RegistrationsPage() {
  ...
  const inputClass =
    'w-full bg-transparent border border-foreground/10 px-4 py-2 text-sm focus:outline-none focus:border-foreground/40 text-foreground font-mono transition-colors';
  const labelClass =
    'block text-[11px] uppercase tracking-wider mb-1.5 text-muted-foreground font-mono';
      ...
      >
        ...
        <div
          className={cn(
            ...
            isFullScreen
              ? 'opacity-0 scale-95 h-0 overflow-hidden mb-0'
              : 'opacity-100 scale-100 mb-10'
          )}
        >
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      return setAlertMsg(
                        'Mohon pastikan seluruh form Kontak diisi dengan benar sebelum melanjutkan ke form Alamat.'
                      );
                    }
                    if (
                      idx === 2 &&
                      ...
                      return setAlertMsg(
                        'Mohon pastikan form Alamat sudah lengkap sebelum melanjutkan.'
                      );
                    }
                    if (
                      idx === 3 &&
                      ...
                      return setAlertMsg(
                        'Mohon lengkapi titik pin lokasi dan detail bangunan Anda.'
                      );
                    }
                    if (idx === 4 && !formData.packageId) {
                      return setAlertMsg(
                        'Mohon pilih Paket Kecepatan WiFi Anda terlebih dahulu.'
                      );
                    }
                    setActiveTab(idx);
                  }}
                  className={cn(
                    ...
                    activeTab === idx
                      ? 'text-foreground bg-foreground/[0.02] font-bold border-foreground'
              ...
              {!isSubmitted ? (
                ...
                <div className="text-center space-y-4 flex flex-col items-center justify-center my-auto min-h-[300px] animate-in zoom-in-95 duration-300">
                  ...
                  <p className="text-sm text-muted-foreground max-w-md leading-relaxed">
                    ...
                    , data pendaftaran kamu aman tersimpan. Mohon ditunggu ya,
                    tim operasional Sicakra WiFi akan segera mengontak WhatsApp
                    kamu untuk mencocokkan jadwal pemasangan!
                  </p>
                  <Link
                    href="/"
                    onClick={() => setIsFullScreen(false)}
                    className="mt-4 px-6 py-3 text-xs font-mono border border-foreground text-background bg-foreground hover:bg-foreground/90 transition-colors"
                  >
                    Kembali ke Beranda
                  </Link>
                </div>
